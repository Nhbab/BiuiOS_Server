...
int install_package(const char *pkg) {
    ...
    // Dependencies (using strtok_r for safe recursive execution)
    if (strlen(deps) > 0 && strcmp(deps, "-") != 0) {
        printf("Resolving dependencies for %s: [%s]\n", pkg, deps);
        char *saveptr;
        char *dep_token = strtok_r(deps, ",", &saveptr);
        while (dep_token != NULL) {
            ...
            dep_token = strtok_r(NULL, ",", &saveptr);
        }
    }
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (stat(abs_path, &st) == 0) {
            if (S_ISDIR(st.st_mode)) {
                rmdir(abs_path); // Safely delete if directory is empty
            } else {
                unlink(abs_path);
            }
        }
